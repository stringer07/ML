...
def convert_eval_output(path, bs):
    """
    Read the evaluation log files and convert them to MMA readable format for further analysis
    :param path:
    :param bs:
    :return:
    """

    evalfile = open(path, "r")
    evalines = evalfile.readlines()

    outpath = path.replace('valid.0', 'valid.cleaned')
    outfile = open(outpath, "w")

    for numl, line in enumerate(evalines):

        if numl % 100 == 0:
            print('Did {} lines out of {}'.format(numl, len(evalines)))

        if numl % (bs+4) == 0 or numl % (bs+4) == bs+3:
            outfile.write(line)
        elif numl % (bs+4) == 1:
            source = evalines[numl].split('src=')[-1][:-1]
            sp_source = sp.mathematica_code(source).replace('citi', 'CiTi')
            outfile.write('src='+sp_source+'\n')
        elif numl % (bs+4) == 2:
            target = evalines[numl].split('tgt=')[-1][:-1]
            sp_target = sp.mathematica_code(target)
            outfile.write('tgt='+sp_target+'\n')
        else:
            val = evalines[numl].split(' ')
            hyp = ' '.join(val[2:])[:-1]

            if 'INT' in hyp or 'add' in hyp:
                outfile.write(val[0] + ' ' + val[1] + ' Failed' + '\n')
            else:
                sp_hyp = sp.mathematica_code(hyp)
                outfile.write(val[0] + ' ' + val[1] + ' ' + sp_hyp + '\n')
