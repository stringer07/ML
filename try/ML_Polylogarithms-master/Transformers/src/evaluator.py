...
"""
Contains routines required to check the validity of an expression
"""
...
from .envs.char_sp import InvalidPrefixExpression, EVAL_VALUES, ZERO_THRESHOLD
from .envs.sympy_utils import simplify
from .envs.wolfram_utils import check_symbol
...
def check_valid_solution(env, src, tgt, hyp):
    ...
    # If we want to evaluate the resulting guess at precise points
    if env.numerical_check:
        complex_args = EVAL_VALUES.copy()
        random.shuffle(complex_args)
        valid_points = []

        # Generate a random complex point
        for arg_num, argument in enumerate(EVAL_VALUES):
            full_arg = argument + sp.I*complex_args[arg_num]
            valid_point = True if abs(sp.N(hyp.subs(x, full_arg)-tgt.subs(x, full_arg))) < ZERO_THRESHOLD else False
        ...
        # If we want to compute the symbol equivalence in python directly (calling a mathematica kernel)
        if env.symbol_check:
            valid, diff_symb = check_symbol(hyp, tgt, env.session)
        
            if not valid:
                print("No match for {}, residual symbol is {} ".format(tgt, diff_symb))
        else:

            # Check the symbolic matching
            valid = simplify(hyp - tgt, seconds=5) == 0
        if not valid and not env.symbol_check:
            diff = simplify(src, seconds=1)
            valid = diff == 0
